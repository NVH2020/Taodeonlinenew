...
const ExamPortal: React.FC<ExamPortalProps> = ({ grade, onBack, onStart }) => {
  ...
  const allAvailableCodes = useMemo(() => {
    const defaults = EXAM_CODES[grade as any] || [];
  ...
  const getRelatedGrades = (currentGrade: string | number) => {
    const cg = currentGrade.toString();
    if (cg === "12") return ["10", "11", "12"];
    if (cg === "11") return ["10", "11"];
    if (cg === "10") return ["10"];
    if (cg === "9") return ["6", "7", "8", "9"];
    if (cg === "8") return ["6", "7", "8"];
    if (cg === "7") return ["6", "7"];
    if (cg === "6") return ["6"];
    return [cg];
  };
  ...
  const combinedTopics = useMemo(() => {
    ...
    relatedGrades.forEach(g => {
      ...
      // Fix: Ensure id is always string and grade is string for consistent mapping
      topics = [...topics, ...gradeTopics.map(t => ({ id: t.id.toString(), name: t.name, grade: g.toString() }))];
    });
    return topics;
  }, [grade, configReady]);
    ...
    />
              ...
              <div className="p-4 space-y-4">
                {(currentCodeDef?.topics as string[])?.map(tid => {
                  // Fix: Fixed tid logic for mixed string/number scenarios
                  const tidStr = tid.toString();
                  const topic = TOPICS_DATA[Math.floor(parseInt(tidStr)/100)]?.find(t => t.id.toString() === tidStr);
                  return topic ? (
                    <div key={tidStr} className="p-5 bg-white rounded-3xl border border-blue-50 shadow-sm flex items-center gap-4">
                      <div className="w-10 h-10 rounded-2xl bg-blue-50 text-blue-600 flex items-center justify-center font-black text-xs shrink-0">{tidStr}</div>

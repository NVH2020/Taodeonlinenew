...
const LandingPage: React.FC<LandingPageProps> = ({ onSelectGrade, onSelectQuiz, user, onOpenAuth, onOpenVip }) => {
  ...
  // Logic nhập tay cho Trường và Ngân hàng
  ...
  const handleStartQuiz = (e: React.FormEvent) => {
    ...
    // Gửi dữ liệu đi (Thêm dấu nháy đơn vào SĐT và STK để tránh mất số 0 trong Sheet)
    onSelectQuiz(showQuizModal!.num, showQuizModal!.pts, {
      name: quizInfo.name,
      class: quizInfo.class,
      school: quizInfo.school,
      phoneNumber: "'" + quizInfo.phone, // Mẹo giữ số 0
      stk: "'" + bankInfo.stk,           // Mẹo giữ số 0
  ...
  return (
    <div className="flex flex-col gap-6 pb-12 font-sans overflow-x-hidden">
      {/* 1. Header & Marquee (Giữ nguyên như bản của bạn) */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
            ...
            <div className="p-2 space-y-1 flex-grow bg-slate-50 overflow-y-auto max-h-[420px] custom-scrollbar">
              {stats.top10.length > 0 ? (
                stats.top10.map((item, index) => (
                  <div key={index} className="flex items-center justify-between p-2 bg-white rounded-xl border border-slate-100 shadow-sm">
                    <div className="flex flex-col gap-0.5 min-w-0 flex-1 pr-1">
                      <span className="font-bold text-slate-800 text-[10px] truncate">{index + 1}. {item.name || "Học sinh"}</span>
        ...
        {/* CỘT GIỮA & PHẢI (Giữ nguyên ảnh và nút) */}
        ...
        <div className="lg:col-span-2 flex flex-col gap-3">
          <button onClick={onOpenAuth} className="w-full flex-1 flex flex-col items-center justify-center gap-1 bg-indigo-600 text-white rounded-2xl font-black text-[10px] uppercase border-b-4 border-indigo-900 p-2 text-center">
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/90 backdrop-blur-md">
          <div className="bg-white w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl relative border border-slate-100 overflow-y-auto max-h-[95vh]">
            ...
            <form onSubmit={handleStartQuiz} className="space-y-3">
              <input required type="text" placeholder="Họ và tên" className="w-full p-3 bg-slate-50 rounded-xl font-bold outline-none" value={quizInfo.name} onChange={e=>setQuizInfo({...quizInfo, name: e.target.value})} />
              ...
              {/* CHỌN TRƯỜNG */}
              {!isOtherSchool ? (
                <select className="w-full p-3 bg-slate-50 rounded-xl font-bold outline-none" onChange={(e) => e.target.value === "Khác" ? setIsOtherSchool(true) : setQuizInfo({...quizInfo, school: e.target.value})}>
              ...
              <div className="p-4 bg-orange-50 rounded-2xl space-y-3 border border-orange-100">
                <input type="text" placeholder="Số tài khoản ngân hàng" className="w-full p-3 bg-white rounded-xl font-bold outline-none" value={bankInfo.stk} onChange={e => setBankInfo({...bankInfo, stk: e.target.value})} />
                {!isOtherBank ? (
              ...
              <button className="w-full py-4 bg-orange-500 text-white rounded-xl font-black uppercase shadow-lg">Vào thi ngay</button>
            </form>
